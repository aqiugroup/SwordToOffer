...
void test_logger_mask();
void test_logger_mask2();
void test_logger();
...
void test_standard_logger() {
    ...
    StartMS();
	Trace trace(INFO_, "test_standard_logger()");
	...
	{
        Trace trace(INFO_, "test namespace functions");
		string message_suffix(" message through namespace functions");
        debug("debug" + message_suffix);
        info("info" + message_suffix);
		warning("warning" + message_suffix);
		error("error" + message_suffix);
		critical("critical" + message_suffix);
		log(DEBUG_,
				"debug message through log(level, message)");
        log(CRITICAL_,
				"critical message through log(level, message)");
	}

	std::cout << "qzc " << EndMS() << std::endl;
}

void test_logger_mask() {

    using namespace MyLog;
    log(INFO_) << "this is info" << endl;
    log(CRITICAL_) << "this is info" << endl;
    log(WARNING_) << "this is warning" << endl;
    log(INFO_) << "this is info" << endl;
    log(ERROR_) << "this is error" << endl;
}
void test_logger_mask2() {
...
void test_logger_functions(Logger& logger) {
	...
	logger.log(DEBUG_, "debug message through Logger::log(level, message)");
	logger.log(CRITICAL_,
			"critical message through Logger::log(level, message)");
}
...
void test_named_logger() {
	...
	double testnum = 1.234;
	nlog(ERROR_) << testnum << " hello " << endl;

	nlog = named_logger("module2");
